...
- [verification-report.md](verification-report.md) records the release, clean installs, package
  checks, authenticated live scaffold, and privacy boundary.
...
- [terminal-demo.sh](terminal-demo.sh) installs v0.4.1 in isolation, previews the prompt, runs a real
  backend, and independently checks the generated project.
...
1. A fresh uv tool directory installs v0.4.1 from its immutable GitHub archive and exposes
   `forge`.
2. The public Homebrew tap installs and tests v0.4.1 from source.
...
- Measured outputs are dated in the verification report.
